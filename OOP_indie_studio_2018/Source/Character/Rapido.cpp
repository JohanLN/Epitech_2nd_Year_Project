...
Rapido::Rapido(std::string playerNb, int x, int y) : ACharacter("Rapido", x, y)
{
    _playerNb = playerNb;
...
void Rapido::particulesExplosion(scene::ISceneManager *manager, video::IVideoDriver *driver)
{
...
bool Rapido::timeBeforeParticules()
{
    auto moment = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsedTime = moment - _start;
...
void Rapido::movement(IrrlichtDevice *device, scene::ISceneManager *manager, video::IVideoDriver *driver, Map *map)
{
    bool check = false;
    int bombPress = 0;
...
core::vector3df Rapido::getPlayePos() {
    return _playerPos;
}

void Rapido::setPlayerAt() {
...
void Rapido::createMesh(scene::ISceneManager *manager, video::IVideoDriver *driver, int nbPlayer) {
    ...
    _mesh->setMaterialTexture(0, driver->getTexture("Resources/Media/player_blue.png"));
    ...
    _run->setMaterialTexture(0, driver->getTexture("Resources/Media/player_blue.png"));
...
void Rapido::setPlayerPos(const core::vector3df &playerPos) {
