...
Adc::Adc(std::string playerNb, int x, int y) : ACharacter("Adc", x, y)
{
    _playerNb = playerNb;
...
void Adc::particulesExplosion(scene::ISceneManager *manager, video::IVideoDriver *driver)
{
...
void Adc::movement(IrrlichtDevice *device, scene::ISceneManager *manager, video::IVideoDriver *driver, Map *map) {
    bool check = false;
    int bombPress = 0;
...
core::vector3df Adc::getPlayePos() {
    return _playerPos;
}

void Adc::setPlayerAt() {
...
void Adc::createMesh(scene::ISceneManager *manager, video::IVideoDriver *driver, int nbPlayer) {
    ...
    _mesh->setMaterialTexture(0, driver->getTexture("Resources/Media/player_black.png"));
    ...
    _run->setMaterialTexture(0, driver->getTexture("Resources/Media/player_black.png"));
...
void Adc::setPlayerPos(const core::vector3df &playerPos) {
